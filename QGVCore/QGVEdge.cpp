...
void QGVEdge::updateLayout()
{
        ...
        if ((spl->list != 0) && (spl->list->size % 3 == 1)) {
                if (spl->list->sflag) {
                        _tail_arrow = toArrow(QLineF(QGVCore::toPoint(spl->list->list[0], gheight),
                                                     QGVCore::toPoint(spl->list->sp, gheight)));
                }

                if (spl->list->eflag) {
                        _head_arrow = toArrow(QLineF(QGVCore::toPoint(spl->list->list[spl->list->size - 1], gheight),
...
void QGVEdge::setTailport(const QString &port)
{
        this->setAttribute("tailport", port);
}

void QGVEdge::setHeadport(const QString &port)
{
        this->setAttribute("headport", port);
}
