...
void QGVScene::loadLayout(const QString &text)
{
        ...
        for (Agnode_t *node = agfstnode(_graph->graph()); node != NULL; node = agnxtnode(_graph->graph(), node)) {
                ...
                inode->updateLayout();
                addItem(inode);
                for (Agedge_t *edge = agfstout(_graph->graph(), node);
                     edge != NULL; edge = agnxtout(_graph->graph(), edge)) {
...
//        gvRenderFilename(_context->context(), _graph->graph(), "svg", "debug.svg");
        ...
        if (xlabel) {
                ...
                item->setPos(
                        QGVCore::centerToOrigin(QGVCore::toPoint(xlabel->pos, QGVCore::graphHeight(_graph->graph())),
                                                xlabel->dimen.x, -4));
        }
